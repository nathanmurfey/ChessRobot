...
void loop() {
  ...
  if(Serial.available()){
    ...
    finalConditions[0] = 307;
    finalConditions[1] = 219;
    finalConditions[2] = 214;
    finalConditions[3] = 0;
    finalConditions[4] = 0;
    finalConditions[5] = 0;

    trajectory.TrajectoryBuild(initConditions, finalConditions, 5);
  ...
  motorOne.UpdateDesiredVelocity(Q1Dot_d);
  motorTwo.UpdateDesiredVelocity(Q2Dot_d);
  motorThree.UpdateDesiredVelocity(Q3Dot_d);
  motorFour.UpdateDesiredVelocity(Q4Dot_d);

  motorOne.CheckStationary(motorOne.getTicks(),1);
  motorTwo.CheckStationary(motorTwo.getTicks(),2);
  motorThree.CheckStationary(motorThree.getTicks(),3);
  motorFour.CheckStationary(motorFour.getTicks(),4);

  Serial.print("Motor one: ");
  motorOne.RunMotorController(1, 0);
